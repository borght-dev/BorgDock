# PRDock — GitHub PR Monitor & Auto-Fix Tool
...
Location: `%APPDATA%\PRDock\settings.json`
...
When no `settings.json` exists, PRDock launches a multi-step setup wizard:
...
1. **Title row** — "PRDock" label, pin/unpin toggle, minimize-to-badge button, settings gear icon.
...
3. PRDock runs `git worktree remove {path}` for each selected worktree.
...
2. Build a prompt file (`.md`) and save to `%APPDATA%\PRDock\prompts\{timestamp}-PR{number}-{check_name}.md`. Contents:
   - PR title, number, URL.
...
Prompt files are stored in `%APPDATA%\PRDock\prompts\` with timestamps for debugging and history. Files older than 7 days are automatically pruned on app startup.

### Custom Prompts — Layered Approach

Claude Code prompts are layered:

1. **Repo's own `CLAUDE.md`** — Claude Code reads this automatically from the worktree root. PRDock doesn't need to do anything.
2. **Per-repo `fixPromptTemplate`** — If configured in settings, PRDock injects this text into the prompt file. Example: "Always run `pnpm test` after fixing. Use pnpm, not npm."
3. **PRDock's standard error context** — PR details, parsed errors, raw log.
...
- **System tray shows active session count** — Tray tooltip updates to: "PRDock — 12 PRs, 2 fixing".
...
- Tray icon with tooltip showing summary: "PRDock — 12 open PRs, 2 failing" (+ ", 1 fixing" if Claude Code sessions are active).
...
├── src/
│   └── PRDock.App/
│       ├── PRDock.App.csproj          (.NET 10, WPF, single-file publish)
...
| Prompt file storage | %APPDATA%\PRDock\prompts\, auto-prune >7 days | Debugging history without unbounded growth |

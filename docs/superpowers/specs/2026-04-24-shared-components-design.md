...
| #9 | `feat/streamline-09-visual-routes` | Planned | — | TBD | Visual surface URL routing (`?section`/`?settings`/`?wizard`/`?toast=`) so visual.spec.ts can capture per-surface screenshots; deferred from PR #8 to keep scope bounded (4 cross-component integration points + pre-existing settings.spec.ts:11 failure path). |

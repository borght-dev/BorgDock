...
| #5 | `feat/streamline-05-palettes` | In review | — | 2026-04-25 | Palettes + viewers migration: `command-palette/{CommandPalette, PaletteApp, PaletteRow}`; `file-palette/{FilePaletteApp, RootsColumn, SearchPane, ResultsList, PreviewPane, ChangesSection, CodeView}`; `file-viewer/{FileViewerApp, FileViewerToolbar}`; `worktree-palette/WorktreePaletteApp`; `worktree/WorktreePruneDialog`; `sql/{SqlApp, ResultsTable}` — all migrated to PR #1 primitives (Avatar, Pill, Chip, Dot, Ring, Button, IconButton, Card, Input, LinearProgress, Kbd). Per-feature CSS deleted: `styles/file-palette.css`, `styles/file-viewer.css`, `styles/worktree-palette.css`, `components/file-palette/CodeView.css` — residual layout ported into `@layer components` in `index.css` under `bd-fp-*` / `bd-fv-*` / `bd-wt-*` / `bd-code-*` namespaces (606 lines added). Per spec §4.2 **no per-feature `.css` files remain in the repo** after this PR. The last in-scope `--color-filter-chip-bg` JSX consumer outside `settings/**` (the `.wt-group-count` chip in `WorktreePaletteApp`) replaced with `<Pill tone="ghost">` — token itself stays in `index.css` for PR #6 to delete. Test-contract `data-*` hooks added: `data-file-result`/`data-selected`, `data-file-preview`, `data-window="palette"`, `data-titlebar-path`, `data-line-gutter`/`data-line-number`, `hl-*` className on syntax tokens, `data-action="copy-contents"`, `data-worktree-row`/`data-tree-path`/per-action `data-action`. CodeView syntax highlighting (web-tree-sitter) untouched. **2593 vitest pass** (-13 vs PR #4 baseline 2606 — Task 13's WorktreePruneDialog test-file overwrite dropped 13 wrapper-style assertions covering empty/loading/path-truncation behaviors that targeted a `TestDialog` shim rather than the real component; net coverage is now against the real component but with fewer permutations — flagged for PR #6 follow-up). `npx vite build` clean; zero `[plugin:vite:css]` errors. E2e behavioral specs gained `file-viewer.spec.ts:14/18/25/31` + `file-palette.spec.ts:14/37` + `worktree-palette.spec.ts:12` data-* contracts — generic `Cmd+K` command palette and per-row prune/checkout actions remain pre-existing aspirational gaps in `command-palette.spec.ts` / `worktree-palette.spec.ts:16/22` (out of migration scope; belongs in a future palette-feature PR). Visual baselines remain red as expected per `visual.spec.ts` header — `clipTo` selectors + URL-routed deep-links infra still out of scope. Opened as stacked PR against `feat/streamline-04-pr-detail` — https://github.com/borght-dev/BorgDock/pull/6. |
| #6 | `feat/streamline-06-ancillary` | In review | — | 2026-04-25 | Ancillary surfaces migration: every consumer in `components/settings/**` (AdoSection, AppearanceSection, ClaudeApiSection, ClaudeSection, GitHubSection, HotkeyRecorder, NotificationSection, RepoSection, SettingsFlyout, SqlSection, UpdateSection), `components/wizard/**` (AuthStep, RepoStep, SetupWizard — wizard now has a third "Customize Appearance" step persisting `sidebarEdge` + `theme`), `components/onboarding/**` (FeatureBadge, FirstRunOverlay, InlineHint), `components/notifications/**` (NotificationBubble, NotificationManager, NotificationOverlay), and `components/whats-new/**` (AlsoFixedList, HeroBanner, HighlightCard, ReleaseAccordion, WhatsNewApp) migrated onto PR #1 primitives (Avatar, Pill, Chip, Dot, Ring, Button, IconButton, Card, Input, LinearProgress, Kbd). New feature-local `_ToggleSwitch` lives next to settings sections; not a primitive. **Floating Badge** (NotificationBubble — 5 variants) rebuilt on Card frame + IconButton dismiss with `shadow-[var(--elevation-2)]` utility + severity-driven `--color-toast-{severity}-glow` retained inline (justified — token name is dynamic). **Final §4.3 style sweep** lands: 172 → 109 occurrences of `style={{}}` (63 static values converted to Tailwind utilities, 109 dynamic values annotated with `// style: …` justification comments — virtualizer absolute positioning, color-mix blends, raf-tweened maxHeight, custom keyframes, var(--font-code), sub-pixel borders, context-menu anchor coords, severity/tier-driven token names). **Token deletion**: `--color-filter-chip-bg` / `--color-filter-chip-fg` removed from `index.css` (`@theme inline` + light + dark — 6 lines) after migrating every settings consumer; `.sidebar-open-badge` `@layer-components` selector repointed at `--color-surface-hover`. **Restored 18 wrapper-style WorktreePruneDialog test permutations** PR #5 Task 13 dropped (empty/loading/path-truncation×2/plural/singular/refs-heads-stripping/multi-select-count/error/progress/disabled-during-removal + helper-function suites for statusLabel/statusClasses/truncatePath); kept PR #5's 2 direct-component dialog-shell tests. **Fixed `--color-text-muted` contrast** to pass WCAG 2.1 AA against effective surface-raised: light `#8a85a0` → `#6a6580` (3.39:1 → 5.32:1), dark `#5a5670` → `#9490a8` (2.43:1 → 5.52:1). Added `src/styles/__tests__/contrast.test.ts` regression guard; dropped `disableRules: ['color-contrast']` from `tests/e2e/whats-new.spec.ts`. **Wizard `getByText('Access Token')` strict-mode disambiguation** (Task 10 follow-up): subtitle changed from "Enter a personal access token" → "Paste your GitHub token" so the e2e locator no longer matches two elements. Test-contract `data-*` hooks added: `data-toast` (kept), `data-notification-severity`, `data-notification-overlay`, `data-feature-badge`/`-badge-id`, `data-onboarding-hint`/`-hint-id`, `data-first-run-overlay`, `data-release-version`, `data-fixed-accordion`/`-open`, `data-highlight-kind`/`-pill-tone`, `data-also-fixed-list`, `data-whats-new-app`, `data-wizard-step`/`-action`/`-overlay`, `data-auth-method`/`-status`, `data-repo-row`/`-name`, `data-settings-section`/`-control`/`-action`/`-close`/`-overlay`, `data-flyout="settings"`, `data-toggle`/`-checked`, `data-hotkey-recorder`/`-recording`, `data-notification-type`. **2640 vitest pass** (+47 vs PR #5 baseline 2593, 0 failures, 0 unhandled rejections — same async-mock cluster from PR #5 baseline (`onboarding-store.test.ts`) is no longer surfacing as the 8 cross-test errors due to test-execution-order shifts; not introduced or fixed by PR #6 directly). `npx vite build` clean; zero `[plugin:vite:css]` errors. e2e behavioral specs (settings, setup-wizard, notifications, whats-new) green; `whats-new > a11y` now passes natively without rule disablement. Visual baselines remain pending the `clipTo` infra gap (out of scope per spec §2 + PR #0 infra deferral). Worktree Changes feature (§6) deferred to PR #7. Pre-existing TS errors in `DiffFileSection.tsx` (PR #4 work) and one in `WorktreePruneDialog.test.tsx:307` (`HTMLElement \| undefined` from `getAllByRole(...)[0]` in the restored Task 1 wrapper suite) noted; vitest still passes — TS strictness cleanup can land in a follow-up PR. Opened as stacked PR against `feat/streamline-05-palettes` — https://github.com/borght-dev/BorgDock/pull/7. |
| #7 | `feat/streamline-07-worktree-changes` | In review | — | 2026-04-26 | Worktree Changes feature: three new git2-backed `#[tauri::command]` functions (`list_worktree_changes`, `diff_worktree_vs_head`, `diff_worktree_vs_base`) under `git/worktree_changes.rs` with base-branch resolution chain (`origin/HEAD → init.defaultBranch → main → master`) + edge-case handling for detached HEAD, missing merge-base, binary files, untracked files, submodules. New `components/worktree-changes/` module with `WorktreeChangesPanel`, `WorktreeChangeRow`, and `WorktreeDiffOverlay`. Integrated into `WorktreePaletteApp` as a `Worktrees` / `Changes` tab strip (every existing `data-worktree-row` / `data-action` contract preserved). `DiffFileSection` minimally refactored to accept pre-parsed hunks (additive — no PR-side consumer changes). Pre-flight Cargo darwin baseline unblocked via `macos-private-api` Tauri feature override on darwin (one-line `[target.'cfg(target_os = "macos")'.dependencies]` block). `git2 = "0.19"` added with `vendored-libgit2`. Test-contract `data-*` hooks added: `data-worktree-changes-panel`, `data-changes-section="vs-head\|vs-base"`, `data-file-change`/`-status`, `data-base-branch`, `data-detached-head`, `data-worktree-diff-overlay`, `data-diff-source-toggle`, `data-diff-source="vs-head\|vs-base"`, `data-worktree-palette-tabs`, `data-worktree-changes-tab-body`. Rust unit tests cover every base-branch fallback case; 10 integration tests cover clean / modified / added / deleted / renamed / binary / untracked / submodule / detached-HEAD / shallow-no-merge-base. Vitest 2640 → 2663 (Δ from new component + service tests, 0 failing). 5 pre-existing `file_palette::{cache,content_search}` cargo test failures (masked by the prior darwin compile error) surface now that the build is clean — out of PR #7 scope; flagged for follow-up. New e2e spec `worktree-changes.spec.ts` is wired against the Tauri-mock invoke layer; behavioral assertions are blocked by the same `waitForAppReady` infrastructure gap that affects `worktree-palette.spec.ts` on PR #6 — out of scope per spec §2 + PR #0 infra deferral. Visual baselines remain pending the `clipTo` infra gap. Opened as stacked PR against `feat/streamline-06-ancillary` — https://github.com/borght-dev/BorgDock/pull/8. |
| #8 | `feat/streamline-08-test-infra` | In review | — | 2026-04-26 | Test infrastructure cleanup. **Cargo:** 5 pre-existing `file_palette::{cache,content_search}` failures fixed at the implementation level — `normalize_root` now lowercases drive-letter paths cross-OS so the cache-key contract is honored on darwin (recovered 2 tests); `search_content::search()` now takes a cancellation closure instead of reading the static `CURRENT_TOKEN`, so cargo's parallel test execution stops trampling the global atomic (recovered 3 tests). `cargo test --lib` is now 73 pass / 0 fail (was 68 / 5). **E2E mock layer:** corrected wrong window URLs in 5 specs (`/palette.html?kind=…` → real `/worktree.html` / `/file-palette.html`; `?file=` → `?path=` for file-viewer); extended `TAURI_MOCK_SCRIPT` with `plugin:window|*` IPC handlers (`inner_size`, `scale_factor`, `current_monitor`, `set_size`, `close`, `start_dragging`) + `plugin:event|listen|unlisten` + `palette_ready` / `open_in_terminal` / `open_in_editor` so chromeless apps mount silently; added content-bearing mocks for the actual Rust commands the apps invoke (discovered to be `list_root_files`, `read_text_file`, `git_file_diff`, `search_content`, `git_changed_files`, `open_file_viewer_window` — NOT the placeholder names from the original plan); added a `seedPrDetail()` helper in `tests/e2e/helpers/seed.ts` that injects `__BORGDOCK_PR_DETAIL__` so `diff-viewer.spec.ts` mirrors production initialization_script behavior; extended the GitHub fetch interceptor with realistic `/pulls/714` + `/pulls/714/files` (multi-hunk patch) + `/pulls/714/commits` + `/pulls?state=open` fixtures, gated on `__BORGDOCK_PR_DETAIL__` presence so other specs are unaffected; added missing `claudeApi.apiKey` to `completedSettings()` (OverviewTab unconditionally reads it). Hunk-nav assertion in `diff-viewer.spec.ts` adapted to use `getBoundingClientRect().top` against the overflow-y-auto pane (the patch fixture was deliberately built with 25+ context lines between hunks so the second hunk overflows the viewport). **Visual baselines:** tightened `ready` selectors per surface — chromeless windows wait on `[data-tauri-drag-region]` (PaletteApp's drag handle gained the attribute to match), main-window surfaces wait on `<header>`. Flyout fell back to `body` (FlyoutGlance/FlyoutInitializing have no drag-region — documented as a follow-up gap). Added `clipTo: '.bd-wt-palette'` to the worktree-changes surface for a stable rect that excludes scrollbars; other chromeless surfaces fill the whole viewport so no tighter clip exists. **TS strictness:** narrowed `DiffFileSection.tsx`'s 2 `NodeListOf<Element>` index access errors (lines 73/85, both `(target ?? headers[0]).scrollIntoView`) and `WorktreePruneDialog.test.tsx:307`'s `getAllByRole(...)[0]` `HTMLElement \| undefined` access — `tsc --noEmit` clean for both files. **Visual surface URL routing** (`?section`/`?settings`/`?wizard`/`?toast`) deferred to PR #9 with a concrete plan file at `docs/superpowers/plans/2026-04-26-streamline-pr09-visual-routes.md` (4 cross-component integration points + a pre-existing `settings.spec.ts:11` failure path that would compound the deferral if left in scope). **Vitest 2663 pass / 0 fail** (unchanged — TS narrowings are no-ops at runtime). **Playwright historic 5-spec batch (worktree-palette, worktree-changes, file-palette, file-viewer, diff-viewer): 18 pass / 7 fail** (was 7 / 20 on PR #7 baseline, +11 net recovery). Remaining 7 fails are infra-deferred: 5 a11y color-contrast in chromeless windows (separate from PR #6's main-shell contrast fix), 1 `file-palette escape closes palette` (`getCurrentWindow().close()` can't be simulated in plain browser), 1 `worktree-palette prune action opens confirm dialog` (pre-existing aspirational gap from PR #5). `window-rendering.spec.ts` improved from 0/11 → 9/11 PR-detail/Flyout smoke tests (unintentional benefit of the B2/B3/B4 mock work). Opened as stacked PR against `feat/streamline-07-worktree-changes` — https://github.com/borght-dev/BorgDock/pull/9. |
| #9 | `feat/streamline-09-visual-routes` | In review | — | 2026-04-26 | Visual surface URL routing: dev/test-guarded `?section=focus\|prs\|work-items` (Header), `?settings=open` (App), `?wizard=force` (App `needsSetup` gate via module-scope `forceWizardFromUrl` IIFE), `?toast=test` (NotificationOverlay) deep-links so `tests/e2e/visual.spec.ts` mounts each per-surface screenshot on the actual target view instead of the default sidebar. Production navigation unaffected — guard is `import.meta.env.DEV \|\| window.__PLAYWRIGHT__`; the `__PLAYWRIGHT__` runtime escape hatch is set by the `injectCompletedSetup` Playwright helper (`window.__PLAYWRIGHT__ = true` via `addInitScript`). `__PLAYWRIGHT__` consolidated into the existing `declare global { interface Window }` block in `src/test-support/test-seed.ts` — the four mount-effect call sites consume it as `window.__PLAYWRIGHT__` directly. `data-section={activeSection}` exposed on Sidebar's content wrapper for stable per-section ready-selectors. **Pre-existing `settings.spec.ts:11`** ("clicking settings icon opens the flyout") root-caused: the click → `setSettingsOpen(true)` → flyout-render chain has always worked; `:11` was failing on Save/Cancel button assertions for buttons that commit `423cfc7e` removed in favor of debounced auto-save + `Close settings` IconButton + Escape dismiss. Replaced with assertions against the current production contract (`[data-flyout="settings"]` visibility, `Settings` header span, `Close settings` IconButton). **Cancel/Save tests in same file** (`:36` and the Save test) had identical root cause and were dropped — open-and-close round-trip already covered by `:11`; save-on-close debounced timer + flush effect at `SettingsFlyout.tsx:39-56` belongs at the unit-test layer (vitest fake timers). 4 other stale `settings.spec.ts` tests (theme toggle, sidebar edge, hotkey recorder, run-at-startup) have separate stale-spec issues and remain out of scope for follow-up. **`flyout` SURFACE decision:** kept `body` as the permanent ready selector — the flyout is a floating glance card, not a movable windowed app, so `data-tauri-drag-region` would be semantically inappropriate; pixel diff is the actual progress signal. The PR #8 "follow-up" hedge in the SURFACE note is removed. **Vitest 2663 pass / 0 fail** (unchanged from PR #8 baseline — frontend-only routing additions, no logic change). **Cargo 73 pass / 0 fail** (no Rust changes — `macos-private-api` darwin override carried through). **Historic 5-spec Playwright batch 18 pass / 7 fail per project** (unchanged — same infra-deferred fail set as PR #8 baseline: 5 a11y color-contrast in chromeless windows + 1 `file-palette escape closes palette` `getCurrentWindow().close()` sim limit + 1 `worktree-palette prune action opens confirm dialog` aspirational gap). Visual baselines remain pending the design migration; every URL-routed surface (`focus-tab`, `work-items`, `settings`, `wizard`, `toasts`, `flyout`) now resolves its ready selector — pixel diffs RED is the progress signal, not a PR #9 bug. Opened as stacked PR against `feat/streamline-08-test-infra` — https://github.com/borght-dev/BorgDock/pull/10. |

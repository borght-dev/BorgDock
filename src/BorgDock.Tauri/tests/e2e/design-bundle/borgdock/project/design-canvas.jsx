...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  const sectionOrder = [];
  // Flatten fragment-wrapped artboards so helpers that return <>...</> work.
  const flatten = (nodes) => {
    const out = [];
    React.Children.forEach(nodes, (n) => {
      if (!n) return;
      if (n.type === React.Fragment) out.push(...flatten(n.props.children));
      else if (n.type && n.type !== DCSection && n.type !== DCArtboard && typeof n.type === 'function') {
        // Call function components once to unwrap LightDarkPair-style helpers.
        try { out.push(...flatten(n.type(n.props))); } catch { out.push(n); }
      } else out.push(n);
    });
    return out;
  };
  React.Children.forEach(children, (sec) => {
    ...
    const srcIds = [];
    flatten(sec.props.children).forEach((ab) => {
...
function DCSection({ id, title, subtitle, children, gap = 48 }) {
  ...
  const sid = id ?? title;
  // Same flatten behaviour as DesignCanvas so rendered order matches registry.
  const flatten = (nodes) => {
    const out = [];
    React.Children.forEach(nodes, (n) => {
      if (!n) return;
      if (n.type === React.Fragment) out.push(...flatten(n.props.children));
      else if (n.type && n.type !== DCArtboard && typeof n.type === 'function') {
        try { out.push(...flatten(n.type(n.props))); } catch { out.push(n); }
      } else out.push(n);
    });
    return out;

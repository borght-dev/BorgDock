...
describe('FilePaletteCodeView', () => {
  ...
  it('renders one row per line with 1-based line numbers', () => {
    render(<FilePaletteCodeView path="test.ts" content={'a\nb\nc'} />);
  ...
  it('applies the highlightedLines class to the requested rows', () => {
    render(<FilePaletteCodeView path="t.ts" content={'a\nb\nc\nd'} highlightedLines={[2, 4]} />);
  ...
  it('writes full content to clipboard on Ctrl+Shift+C', async () => {
    ...
    render(<FilePaletteCodeView path="t.ts" content="hello" />);
  ...
  it('row gutter carries data-line-gutter and number carries data-line-number', () => {
    const { container } = render(<FilePaletteCodeView path="src/foo.ts" content={'line1\nline2'} />);

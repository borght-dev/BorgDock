...
describe('PrCardView', () => {
  it('renders title, repo, and #number in compact density', () => {
    render(<PrCardView pr={basePr} density="compact" />);
  ...
  it('renders the readiness Ring in normal density when score is provided', () => {
    const { container } = render(
      <PrCardView pr={basePr} density="normal" score={72} />,
    );
  ...
  it('emits data-active="true" when active prop is set', () => {
    const { container } = render(
      <PrCardView pr={basePr} density="compact" active />,
    );
    expect(
  ...
  it('renders the approved review pill with data-pill-tone="approved"', () => {
    const { container } = render(
      <PrCardView pr={{ ...basePr, reviewState: 'approved' }} density="compact" />,
    );
  ...
  it('fires onClick when the row is clicked', () => {
    const onClick = vi.fn();
    const { container } = render(
      <PrCardView pr={basePr} density="compact" onClick={onClick} />,
    );
  ...
  it('renders branch + diff stats only in normal density', () => {
    ...
    const { rerender, container } = render(
      <PrCardView pr={enriched} density="normal" />,
    );
    expect(screen.getByText(/features\/54258/)).toBeInTheDocument();
    expect(screen.getByText(/\+135/)).toBeInTheDocument();
    expect(screen.getByText(/[\u2212-]10/)).toBeInTheDocument();
    rerender(<PrCardView pr={enriched} density="compact" />);
  ...
  it('renders the commented review pill with data-pill-tone="commented"', () => {
    const { container } = render(
      <PrCardView pr={{ ...basePr, reviewState: 'commented' }} density="compact" />,
    );
    expect(
  ...
  it('renders the pending review pill with data-pill-tone="pending"', () => {
    const { container } = render(
      <PrCardView pr={{ ...basePr, reviewState: 'pending' }} density="compact" />,
    );
    expect(
  ...
  it('renders comment count when commentCount is greater than zero', () => {
    render(<PrCardView pr={{ ...basePr, commentCount: 5 }} density="normal" />);
    expect(screen.getByText('5')).toBeInTheDocument();
  });

  it('does not render comment count when commentCount is zero', () => {
    const { container } = render(
      <PrCardView pr={{ ...basePr, commentCount: 0 }} density="normal" />,
    );

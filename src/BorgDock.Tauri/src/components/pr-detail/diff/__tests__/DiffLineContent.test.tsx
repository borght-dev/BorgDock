...
describe('DiffLineContent', () => {
  ...
  it('renders inline changes with priority over syntax spans', () => {
    ...
    const syntaxSpans: HighlightSpan[] = [{ start: 0, end: 5, category: 'keyword' }];
  ...
  it('renders added inline changes with highlighted background', () => {
    const inlineChanges: InlineChange[] = [{ type: 'added', text: 'new text' }];
    const { container } = render(
  ...
  it('renders deleted inline changes with highlighted background', () => {
    const inlineChanges: InlineChange[] = [{ type: 'deleted', text: 'old text' }];
    const { container } = render(
  ...
  it('renders unchanged inline changes without special styling', () => {
    const inlineChanges: InlineChange[] = [{ type: 'unchanged', text: 'same' }];
    const { container } = render(<DiffLineContent content="same" inlineChanges={inlineChanges} />);
    const spans = container.querySelectorAll('span');
  ...
  it('renders text before first syntax span as plain', () => {
    const syntaxSpans: HighlightSpan[] = [{ start: 5, end: 10, category: 'string' }];
    render(<DiffLineContent content="abcde12345rest" syntaxSpans={syntaxSpans} />);
    expect(screen.getByText('abcde')).toBeDefined();
    expect(screen.getByText('12345')).toBeDefined();
  });

  it('renders text after last syntax span as plain', () => {
    const syntaxSpans: HighlightSpan[] = [{ start: 0, end: 3, category: 'type' }];
    const { container } = render(<DiffLineContent content="int main" syntaxSpans={syntaxSpans} />);
    const highlighted = container.querySelector('span[style]');
  ...
  it('renders mixed inline changes: unchanged + added + deleted', () => {
    ...
    const { container } = render(<DiffLineContent content="abc" inlineChanges={inlineChanges} />);
    const spans = container.querySelectorAll('span');
    expect(spans.length).toBe(3);
  });

  it('handles syntax spans that overlap or exceed content length', () => {
    const syntaxSpans: HighlightSpan[] = [{ start: 0, end: 100, category: 'comment' }];
  ...
  it('renders empty content with syntax spans as newline', () => {
    const syntaxSpans: HighlightSpan[] = [{ start: 0, end: 1, category: 'keyword' }];
    const { container } = render(<DiffLineContent content="" syntaxSpans={syntaxSpans} />);

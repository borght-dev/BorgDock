...
use borgdock_lib::git::worktree_changes::{
    diff_worktree_vs_head_inner, list_worktree_changes_inner, BaseBranchSource, FileChangeStatus,
};
...
fn added_file_shows_as_added() {
    ...
    assert!(out
        .vs_head
        .iter()
        .any(|c| c.path == "new.txt" && c.status == FileChangeStatus::Added));
}
...
fn deleted_file_shows_as_deleted() {
    ...
    assert!(out
        .vs_head
        .iter()
        .any(|c| c.path == "README.md" && c.status == FileChangeStatus::Deleted));
}
...
fn untracked_file_is_listed_as_untracked() {
    ...
    assert!(out
        .vs_head
        .iter()
        .any(|c| c.path == "untracked.txt" && c.status == FileChangeStatus::Untracked));
}
...
fn shallow_no_mergebase_marks_merge_base_unavailable() {
    ...
    assert!(matches!(
        out.base_branch_source,
        BaseBranchSource::FallbackMaster
    ));

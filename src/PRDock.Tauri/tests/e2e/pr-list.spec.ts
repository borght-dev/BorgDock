...
test.describe('PR List', () => {
  ...
  test('shows "No pull requests found" when empty', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      const event = new CustomEvent('__prdock_force_state', {
  ...
  test('header contains PRDock logo and section switcher', async ({ page }) => {
    await expect(page.getByText('PRDock')).toBeVisible();

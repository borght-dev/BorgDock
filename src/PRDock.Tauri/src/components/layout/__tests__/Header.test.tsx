...
describe('Header', () => {
  ...
  it('renders the PRDock title', () => {
    render(<Header />);
    expect(screen.getByText('PRDock')).toBeTruthy();
  });
  ...
  it('dispatches prdock-refresh event on Refresh click', () => {
    const spy = vi.fn();
    document.addEventListener('prdock-refresh', spy);
    render(<Header />);
    fireEvent.click(screen.getByLabelText('Refresh'));
    expect(spy).toHaveBeenCalledTimes(1);
    document.removeEventListener('prdock-refresh', spy);
  });

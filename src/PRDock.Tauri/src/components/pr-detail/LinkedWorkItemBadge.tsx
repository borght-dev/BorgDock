...
export function LinkedWorkItemBadge({ workItemId, workItem, compact }: LinkedWorkItemBadgeProps) {
  if (compact) {
    ...
    return (
      <span
        ...
        title={workItem ? `${compactTitle ?? 'Untitled'} (${compactState ?? 'Unknown'})` : `Work Item #${workItemId}`}
      >
        AB#{workItemId}
      </span>
    );
  ...
  const assignedTo = workItem?.fields['System.AssignedTo'] as { displayName?: string } | string | undefined;
    ...
    <div className="flex items-center gap-2 rounded-md border border-[var(--color-subtle-border)] bg-[var(--color-surface-raised)] px-3 py-2">
      ...
      {workItem ? (
        <div className="flex-1 min-w-0">
          <div className="text-xs text-[var(--color-text-primary)] truncate">{title ?? 'Untitled'}</div>

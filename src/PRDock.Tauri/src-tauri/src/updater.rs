...
async fn fetch_latest_json(token: &Option<String>) -> Result<String, String> {
    ...
    // Rewrite platform URLs from github.com to API URLs so the updater can
    // download the .nsis.zip from a private repo with token auth.
            ...
            if let Some(url_val) = platform.get_mut("url") {
                if let Some(url_str) = url_val.as_str() {
                    if let Some(api_url) = url_rewrites.get(url_str) {
                        *url_val = serde_json::Value::String(api_url.clone());
                    }
                }
            }
        }
...
pub async fn download_and_install_update(app: AppHandle) -> Result<(), String> {
    ...
    // Add auth + accept headers for the .nsis.zip download via the API
    if let Some(t) = &token {
        builder = builder
